...
class ExpenseModel:
    def create(self, user_id: int, title: str, amount: float, category: str, date: str, note: str = "", type: str = "expense", account_id: int = 0, to_account_id: int = 0, to_amount: float = None) -> str:
        with get_db() as conn:
            cursor = conn.cursor()
            created_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cursor.execute('''
                INSERT INTO expenses (user_id, title, amount, category, date, note, created_at, type, account_id, to_account_id, to_amount)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (user_id, title, float(amount), category, date, note, created_at, type, account_id, to_account_id, to_amount))
        ...
        with get_db() as conn:
            rows = conn.execute('''
                SELECT
                    ...
                    SUM(amount) as total
                FROM expenses
                WHERE user_id = ? AND category != '股票交易'
    ...
    def update(self, expense_id: str, user_id: int, data: dict) -> bool:
        update_fields = []
        values = []
        for key in ["title", "amount", "category", "date", "note", "type", "account_id", "to_account_id", "to_amount"]:
            if key in data:
                ...
                if key == "amount":
                    ...
                    val = float(data[key]) if data[key] is not None else None
                else:
                    val = data[key]
                values.append(val)
