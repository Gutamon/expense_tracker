...
class AccountModel:
    ...
    def create(self, user_id: int, name: str, icon: str = "💰", type: str = "asset", is_asset: int = 1, billing_start_day: int = 1, currency: str = 'TWD', credit_limit: float = 0) -> str:
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO accounts (user_id, name, icon, type, sort_order, is_asset, billing_start_day, currency, credit_limit) VALUES (?, ?, ?, ?, 0, ?, ?, ?, ?)", (user_id, name, icon, type, is_asset, billing_start_day, currency, credit_limit))
