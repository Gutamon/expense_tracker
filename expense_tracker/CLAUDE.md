...
**Rescue codes** recover a lost cookie without needing the ZIP backup. Each device gets a short human-typeable code (`user.ensure_rescue_code`/`regenerate_rescue_code`, stored in `DATA_DIR/users/rescue_codes.csv`, shown on `/settings`). Submitting it on the onboarding screen (`POST /api/onboarding/restore-device`, excluded from the `bind_device` before_request hook so it doesn't get an auto-created throwaway device first) resolves it back to the original `device_id` and re-sets that device's cookie via `user.set_device_cookie`. Regenerating a code invalidates the previous one. Treat a rescue code as a bearer secret — anyone who has it can access that device's ledger — so it's shown once per session, not emailed or logged. The ZIP export in 設定 remains the fallback if a device folder is ever lost outright.

**iOS home-screen PWA quirks are worked around, not fixed at the source.** iOS routinely drops the httponly `device_id` cookie between cold launches of a standalone PWA, which would otherwise make the app "start over" every open. Mitigations: (1) the rescue code is mirrored into `localStorage` (`rescue_code`) on every authenticated page load (`shell.html`, `settings.html`); (2) the onboarding screen's `autoRecover()` reads it and silently re-attaches via `restore-device` before showing the chooser; (3) the device cookie is set `Secure` when the request arrived over HTTPS (detected via `X-Forwarded-Proto` so it works behind ngrok), which persists more reliably on iOS. **Loop-prevention invariant:** wiping a device (`/api/data/wipe`) must also `user.clear_rescue_code` — otherwise `autoRecover` re-attaches to the now-empty device, which is first-run, which shows onboarding, which auto-recovers again… a permanent blank (the page is hidden during the attempt). `autoRecover` additionally guards with a once-per-session `sessionStorage` flag and a 4s failsafe that always restores visibility. Separately, **file downloads must never navigate the page or `window.open`** in a standalone PWA — that backgrounds the app and freezes its JS timers, leaving buttons stuck. Fetch the file as a blob and click a temporary `<a download>` instead (`downloadBlob()` in `settings.html`, used by both export and the wipe flow).
